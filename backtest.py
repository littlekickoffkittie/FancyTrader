...
def print_stats(trades: List[Trade], label: str = "", timeframe: str = "15m"):
    ...
    exp    = tpnl / len(closed)
    pnl_c  = Fore.GREEN if tpnl >= 0 else Fore.RED
    ...
    print(f"  Total PnL   : {pnl_c}{tpnl:+.4f} USDT{Style.RESET_ALL}")
    print(f"  Expectancy  : {pnl_c}{exp:+.4f} USDT/trade{Style.RESET_ALL}")
...
def print_per_symbol_stats(trades: List[Trade], top_n: int = 20):
    ...
    for row in display:
        ...
        print(f"  {sym:<16} {n:>7} "
              ...
              f"{pnl_c}{pnl:>+10.4f}{Style.RESET_ALL} "
              f"{pnl_c}{exp:>+10.4f}{Style.RESET_ALL}")
...
def print_sweep_results(results: List[SweepResult], top_n: int = 15):
    ...
    for r in results[:top_n]:
        ...
        print(
            ...
            f"{pnl_c}{r.total_pnl:>+9.2f}{Style.RESET_ALL} "
            f"{pf_c}{pf_str:>6}{Style.RESET_ALL} "
            f"{pnl_c}{r.expectancy:>+9.4f}{Style.RESET_ALL} "
    ...
    if args.sweep:
        ...
        if closed:
            ...
            for t in sorted(closed, key=lambda x: x.pnl_usdt or 0)[-40:]:
                pnl_c = Fore.GREEN if (t.pnl_usdt or 0) > 0 else Fore.RED
                print(
                    f"  {t.symbol:<14} {t.direction:>5} {t.score:>6} "
                    f"{pnl_c}{t.pnl_usdt:>+8.4f}{Style.RESET_ALL} "
